...
<div class="container-fluid">
    <div class="table-responsive">
        <div class="card border border-success">
            ...
            <div class="card-body">
                <a href="{{ route('penyakit.create') }}" class="btn btn-success btn-icon mb-2"><i
                        class="fa fa-plus"></i>
                    Tambah Penyakit</a>
                <table class="table table-bordered" id="table" width="100%">
                    ...
                    <tbody>
                        @foreach ($penyakit as $nomor => $item)
                        <tr>
                            <td>{{ $nomor + 1 }}</td>
                            <td>{{ $item['kode'] }}</td>
                            <td>{{ $item['nama'] }}</td>
                            ...
                            <td>
                                <div class="btn-group">
                                    <a href="{{ route('penyakit.show', ['penyakit' => $item['id']]) }}"
                                ...
                                <form id="frm-del-{{ $item['id'] }}"
                                    action="{{ route('penyakit.destroy', ['penyakit' => $item['id']]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
